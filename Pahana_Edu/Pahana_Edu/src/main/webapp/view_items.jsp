...
<%
    ...
    String error = request.getParameter("error"); // notfound | bad_id | server
%>
<!DOCTYPE html>
<html>
<head>
...
<div class="wrap">
    <div class="card">
        <h2>Items <span class="pill"><%= total %> total</span></h2>
        <div class="subtitle">Manage store items and inventory.</div>

        <% if ("created".equals(msg)) { %><div class="alert ok">✅ Item created successfully.</div><% } %>
        <% if ("updated".equals(msg)) { %><div class="alert ok">✅ Item updated successfully.</div><% } %>
        <% if ("deleted".equals(msg)) { %><div class="alert ok">✅ Item deleted successfully.</div><% } %>
        <% if (error != null) { %><div class="alert err">⚠️ <%= error %></div><% } %>

        <div class="toolbar">
            <a class="btn primary" href="<%= request.getContextPath() %>/add_item.jsp">➕ Add Item</a>
            <a class="btn" href="<%= request.getContextPath() %>/dashboard.jsp">🏠 Dashboard</a>
            <label class="search">🔎 <input id="search" type="text" placeholder="Search name, SKU…"></label>
        </div>

        <% if (items == null || items.isEmpty()) { %>
        <div class="alert" style="background:transparent;border-color:var(--border);color:var(--muted)">
            No items found. Click
            <a class="btn primary" style="padding:6px 10px;margin-left:8px" href="<%= request.getContextPath() %>/add_item.jsp">Add New</a>
        </div>
        <% } else { %>
        <div class="table-wrap">
            <table id="itemsTable">
                ...
                <tr>
                    <td><%= i.getId() %></td>
                    <td><%= i.getSku() == null ? "-" : i.getSku() %></td>
                    <td><%= i.getName() %></td>
                    <td><span class="pill"><%= i.getPrice() %></span></td>
                    <td><%= i.getStock() %></td>
                    <td><%= i.getDescription() == null ? "" : i.getDescription() %></td>
                    <td>
                        <div class="actions">
                            <a class="btn" href="<%= request.getContextPath() %>/ItemServlet?action=edit&id=<%= i.getId() %>">✏️ Edit</a>
                            <form class="inline" action="<%= request.getContextPath() %>/ItemServlet" method="post"
